...
func (p *ConnPool) init() error {
	for i := 0; i < p.minConnNum; i++ {
		...
		fmt.Printf("enqueue %s->%s", conn.LocalAddr().String(), conn.RemoteAddr().String())
		p.conns <- conn
	}
	return nil
}
...
func (p *ConnPool) Get() (net.Conn, error) {
	...
	select {
	case conn := <-p.conns:
		fmt.Printf("dequeue %s->%s", conn.LocalAddr().String(), conn.RemoteAddr().String())
...
func (p *ConnPool) Remove(conn net.Conn) error {
	...
	default:
		fmt.Printf("close %s->%s", conn.LocalAddr().String(), conn.RemoteAddr().String())
		return conn.Close()
	}
	return nil
}
...
func (p *ConnPool) createConn() (net.Conn, error) {
	...
	fmt.Printf("create %s->%s", conn.LocalAddr().String(), conn.RemoteAddr().String())
	p.totalConnNum = p.totalConnNum + 1
	return conn, nil
}
